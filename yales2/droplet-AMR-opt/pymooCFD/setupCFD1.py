...
class CFD(optStudy):
    ...
    def postProc(caseDir, var):
        ...
        coor = np.loadtxt(outDatPath, delimiter=',', skiprows=1, usecols=(1,2))    
        
        # interpolate to make data mesh independent 
        interp = interp1d(x, y, kind='nearest')
        n_pts = 1000
        x_new = np.linspace(min(x),max(x), n_pts)
        y_new = interp(x_new)
        coor = np.column_stack((x_new, y_new))
        
        # sort interpolated points?
        
        # load in high quality sim. coordinate data with same number of points
        # coorHQ = np.loadtxt(hqSimDatPath)
        # mean_diff = np.mean(abs(coorHQ-coor))
