...
from pymooCFD.setupOpt import dataDir, setAlgorithm, preProcDir, n_workers, \
                                studyDir, meshSF, baseCaseMS, client
from pymooCFD.util.handleData import loadCP, archive, findKeywordLine, loadTxt
from pymooCFD.util.sysTools import removeDir
# import os
...
def runCornerCases(xl, xu):  # alg):
    ...
    from pymooCFD.util.handleData import runPop
    runPop(lim_perms)
    
    # from pymooCFD.setupCFD import runCase
    # extCaseDir = os.path.join(preProcDir, 'lim_perm_sim')
