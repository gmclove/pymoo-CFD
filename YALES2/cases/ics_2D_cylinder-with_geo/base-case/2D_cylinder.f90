...
subroutine compute_drag_lift()
      ...
      type(grid_t), pointer :: grid
      type(el_grp_t), pointer :: el_grp
      type(data_t), pointer :: drag_ptr, lift_ptr
      type(r1_t), pointer :: drag,lift
      type(data_t), pointer :: vorticity_ptr
      type(r2_t), pointer :: vorticity
      real(wp) :: vortnorm
      ! ------------------------

      grid => solver%first_grid

      call register_grid_data("DRAG",DATATYPE_REAL_NODE_SCALAR,grid, &
                                        dump=.true.,restart=.true.,warning_if_exists=.false., &
                                        new_ptr=drag_ptr)
      call register_grid_data("LIFT",DATATYPE_REAL_NODE_SCALAR,grid, &
                                        dump=.true.,restart=.true.,warning_if_exists=.false., &
                                        new_ptr=lift_ptr)

      call find_data(grid%first_data,"VORTICITY",vorticity_ptr)

!      do n=1,grid%nel_grps
!         el_grp => grid%el_grps(n)%ptr
!         drag => drag_ptr%r1_ptrs(n)%ptr
!         lift => lift_ptr%r1_ptrs(n)%ptr
!         vorticity => vorticity_ptr%r2_ptrs(n)%ptr
!         do ino=1,el_grp%nnode
!            vortnorm=sqrt(sum(vorticity%val(1:grid%ndim,ino)**2))
!            drag%val(ino) = 0.0_WP
!            lift%val(ino) = 0.0_WP
!            if (z1%val(ino).gt.0.001.or.z2%val(ino).gt.0.001.or.z3%val(ino).gt.0.001.or.z4%val(ino).gt.0.001) then
!               norm_vort%val(ino) = vortnorm
!            end if
!         end do
!      end do

end subroutine compute_drag_lift
!=================================================================
